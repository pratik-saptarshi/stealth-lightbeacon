...
This document details the architectural specifications, component boundaries, and non-blocking asynchronous data-flow patterns governing the **Stealth Lightbeacon** engine. It is kept in sync with the `v1.2.7` release train.
...
- `config.describe_mcp_runtime()` surfaces the resolved MCP command, args, and timeout values so startup diagnostics can print the pinned runtime contract directly.
...
5. The current release notes for `v1.2.7` live in `changelog.md`, and the compatibility aliases point to the same history.
